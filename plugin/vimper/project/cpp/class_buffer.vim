...
function! vimper#project#cpp#class_buffer#LoadFile(filename)
  ...
  let l:taglines = []
  for l:ftags in tagfiles()
    let l:lines = s:GetTagsForFile(a:filename, l:ftags)
    if !empty(l:lines)
      call extend(l:taglines, l:lines)
    endif
  endfor
  let l:types = {}
  if !empty(l:taglines)
    let l:typedict = s:ExtractTypes(l:taglines)
      "call writefile(l:taglines, l:t_file)
      ...
      if has_key(l:typedict, "class") && !empty(l:typedict["class"])
          let l:taglines = []
          for l:ftags in tagfiles()
            let l:lines = s:GetTaglines(l:typedict["class"], l:ftags)
            if !empty(l:lines)
              call extend(l:taglines, l:lines)
            endif
          endfor
